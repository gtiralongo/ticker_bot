...
def start(update, context):
    ...
    resumen = "/resumen"
    info = "/infobot"    
    buttons = [[KeyboardButton(resumen)],[KeyboardButton(runOff)], [KeyboardButton(runOn)],  [KeyboardButton(info)]]
...
def main():
    ...
    while get_save_info('action.json')['run'] == 'on':
        ...
        print(f'{Color.CYAN}{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}{Color.RESET}  Tendencia: {Color.CYAN}{trend}{Color.RESET}')
        print("---------------------------------------")
        if state == 'BUY':
            try:
                    ...
                    if indicator["EMA50"] >= indicator["EMA20"] and indicator["EMA20"] >= indicator["EMA10"]:
                        ...
                        if indicator["SMA5"] <= indicator['close']:
                            if indicator["MACD.macd"] < 0 and indicator["RSI"] < 45:
                                ...
                                telegram_bot(f'🚀Orden de compra\n📉Operacion: {order_buy["side"]}\n📊Precio: {order_buy["price"]}\n🪙{order_buy["symbolTicker"][:-3]}: {order_buy["quantity"]}\n💵{symbolTicker[-3:]} : {order_buy["total"]}\n🔢Order ID : {order_buy["order_id"]}\n🪐Profit: {round(valor_compra+(valor_compra*(porcent_gan/100)),info_price["minPrice"])}')
...
# ------------------------------------------------------------------------------------------------------------
            ...
            resumen = (f'🪙{symbolTicker[:-3]}: {quantity["quantitySell"]}\n⚙ Estado: {state}  🕘Grafico: {graf_temp}  \n💹Tendencia: {trend}\n🧾Valor de compra: ${valor_compra} 💲Precio actual: ${price}  Porcentaje: {porcent}%\n💰Profit: {round(valor_compra+(valor_compra*(porcent_gan/100)),info_price["minPrice"])}  Profit %: {round(porcent_gan,2)}%\n⏳{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
            if info_action['resumen'] == 'on':
                telegram_bot(resumen)
                save_info({"resumen":"off"},'action.json')
            ...
            if porcent > porcent_gan:
                ...
                state = 'BUY'
                telegram_bot(f'🚀Vendimos 😎 {order_sell["symbolTicker"][:-3]} 💪 {round(((float(order_sell["price"])-valor_compra)*100)/valor_compra,2)}%\n🔢Order ID : {order_sell["order_id"]}  📉Operacion: {order_sell["side"]}\n🪙{order_sell["symbolTicker"][:-3]}: {order_sell["quantity"]}  💵{order_sell["symbolTicker"][-3:]} : {float(order_sell["total"])}\n📊Precio: {order_sell["price"]}')
                save_info(order_sell,'ordenes.json')
                print("------------------------------------")
                print(f'Esperando compra por indicadores')
            else:
